...
def test_get_intelligence_metrics_success(mock_db):
    ...
    assert result["revenue_reliability_score"] == 73.33
    assert result["revenue_concentration_risk"] == 0.56
    assert result["reliable_revenue"] == 1100.0
    assert result["total_revenue_expected"] == 1500.0
    assert result["actual_revenue"] == 800.0
...
def test_get_intelligence_metrics_no_revenue(mock_db):
    ...
    assert result["total_revenue_expected"] == 0.0
...
def test_get_dashboard_metrics_success(mock_db):
    ...
    assert result["cash_balance"] == 13000
    assert result["cash_runway"] == 6.5
    assert result["burn_rate"] == 2000.0
    assert result["monthly_revenue"] == 5000.0
