...
def _upsert_snapshot(db: Session, org_id: int, snapshot_type: str, monthly_revenue, monthly_expenses):
    """Shared upsert logic for all snapshot types."""
    ...
    snapshot = db.query(FinancialSnapshots).filter(
        ...
        FinancialSnapshots.snapshot_type == snapshot_type
    ).first()
    ...
    if not snapshot:
        ...
        snapshot.monthly_expense = monthly_expenses
...
def _upsert_best(db: Session, org_id: int):
    ...
    # projected = prediction_service.calculate_best(db, org_id)
...
def _upsert_worst(db: Session, org_id: int):
    # TODO: get projected figures from prediction_service
    # projected = prediction_service.calculate_worst(db, org_id)
